...
char const *const GPS_ONOFF_FILE
...
//    sprintf(buff, "$PUBX,41,%d,0003,0003,%d,0*", u, baud);
  ...
  DFR("%s: baud rate sent to device: %s", __FUNCTION__, buff);
...
#if 0
  // set GPS device baud to 115200
  gps_dev_set_baud_rate(s->fd, 38400);
  // set TTY device baud to 115200
  dev_tty_setup(s, GPS_DEV_HIGH_BAUD, 0);
...
/*
    if (newBaud)
    {
        ...
        cfsetispeed(&options, newBaud);
        cfsetospeed(&options, newBaud);

        s->fd = open(KSP5012_CHANNEL_NAME, O_RDWR);
        if ((ret = tcsetattr(s->fd, TCSAFLUSH, &options)) < 0) {
            DFR("%s: failed to set attr with baud at %d", __FUNCTION__, newBaud);
            goto exit;
        }
    }
*/
...
//        cfmakeraw(&options);
//        options.c_lflag = 0;
//        options.c_iflag |= (IGNCR | IXOFF);
//        options.c_cflag &= ~(CSTOPB | CRTSCTS | CSIZE | PARENB);
//        options.c_cflag &= ~(CRTSCTS | CSIZE | PARENB);
//        options.c_cflag |= (CLOCAL | CREAD | CSTOPB);
	...
	options.c_cflag &= ~(CRTSCTS | PARENB | CSIZE);
        ...
        if ((ret = tcsetattr(s->fd, TCSANOW, &options)) < 0) {
            DFR("%s: failed to set attr after baud", __FUNCTION__);
            goto exit;
        }
    }
